...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    queryset = Product.objects.annotate(
        count_purchases=Count("purchases")).annotate(
        total_count=Sum("purchases__count")).order_by(
        "-created_at")
...
class MostPopularProductViewSet(viewsets.ModelViewSet):
    ...
    queryset = Product.objects.annotate(count_sum=Sum("purchases__count",  default=0)).order_by('-count_sum')
